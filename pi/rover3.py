...
# Waypoints
target_lat = [23.8296169, 23.82958, 23.829585, 23.829604, 23.829572, 23.829635, 23.829462]
target_lng = [90.5672889, 90.5672, 90.567145, 90.567239, 90.567179, 90.567265, 90.567005]
current_target = 0
    ...
    try:
        while current_target < len(target_lat):
            ...
            if distance < 1:
                ...
                if distance < 7:
                    new_head = 20
                if abs(heading_error) > new_head:  # More aggressive correction threshold
                    if heading_error > 0:
                        null = 1
                        right = 1
                        if left == 1:
                        ...
                        if right == 1:
                            ...
                            time.sleep(0.5)
                        turn_left()
                elif abs(heading_error) < new_head:  # Subtle correction
                    if heading_error > 0:
                        if null == 1:
                            stop_motors()
                            null = 0
